...
namespace TicketService.Application.CQRS.Handler.TicketListing
{
    public class TicketListingGetByIdQueryHandler : IRequestHandler<TicketListingGetByIdQuery, TicketListingGetByIdResponse>
    {
        ...
        public async Task<TicketListingGetByIdResponse> Handle(TicketListingGetByIdQuery request, CancellationToken cancellationToken)
        {
            ...
            var listing = await _unitOfWork.TicketListings
                .GetAllAsync()
                .Include(x => x.Ticket)
            ...
            try
            {
                // l?y t?t c? listing cùng seller + event
                var listings = await _unitOfWork.TicketListings
                    .GetAllAsync()
                    .Include(x => x.Ticket)
                    .Where(x =>
                        x.SellerUserId == listing.SellerUserId &&
                        x.EventId == listing.EventId &&
                        !x.IsDeleted)
                ...
                var eventResponse = await _eventGrpcClient.GetEventDetailAsync(eventRequest, cancellationToken: cancellationToken);
                ...
                {
                    UserId = listing.SellerUserId.ToString()
                };
                ...
                var dto = new TicketListingDTO
                {
                    ...
                    Ticket = listings.Select(x => new TicketListingTicketDTO
                    {
                        Id = x.Ticket.Id.ToString(),
                        OwnerId = x.Ticket.OwnerId.ToString(),
                        Status = x.Ticket.Status,
                        TicketTypeId = x.Ticket.TicketTypeId.ToString(),
                        Zone = x.Ticket.Zone
                    }).ToList(),
                    ...
                    SellerUser = new TicketListingUserDTO
                    {
                        ...
                        FullName = userResponse.FullName,
                        AvatarUrl = userResponse.AvatarUrl,
                        Gender = int.Parse(userResponse.Gender)
                    },
                    ...
                    Event = new TicketListingEventDTO
                    {
                        ...
                        Tags = eventResponse.Tags != null
                            ? JsonSerializer.Deserialize<List<TagRequest>>(eventResponse.Tags)
                            : new List<TagRequest>(),
